...
void left_motor_reverse (float duty) {
    ...
    LIN2_PULSE = usToU16Ticks(MOTOR_PWM_PERIOD, getTimerPrescale(T2CONbits)) * (1-duty);
}

void left_motor_fwd (float duty) {
    LIN1_PULSE = usToU16Ticks(MOTOR_PWM_PERIOD, getTimerPrescale(T2CONbits)) * (1-duty);
...
void right_motor_reverse (float duty) {
    ...
    RIN2_PULSE = usToU16Ticks(MOTOR_PWM_PERIOD, getTimerPrescale(T3CONbits)) * (1-duty);
}

void right_motor_fwd (float duty) {
    RIN1_PULSE = usToU16Ticks(MOTOR_PWM_PERIOD, getTimerPrescale(T3CONbits)) * (1-duty);
